...
namespace PKServ.Business.Admin
{
    public static class DataFixerImpl
    {
        ...
        public static async Task FixDuplicateUsers(DataConnexion dc)
        {
        ...
        public static async Task FixChangedUsernameInEntries(GlobalAppSettings gas, DataConnexion dc)
        {
            var entries = dc.GetAllEntries();
            int count = 0;
            List<User> users = dc.GetAllUserPlatforms();
            entries.ForEach(checkedEntry =>
            {
                if (users.Any(a => a.Code_user == checkedEntry.code) && checkedEntry.Pseudo != users.First(a => a.Code_user == checkedEntry.code).Pseudo)
                {
                    checkedEntry.Pseudo = users.First(a => a.Code_user == checkedEntry.code).Pseudo;
                    checkedEntry.Validate(false);
                    count++;
                }
            });
            if (count > 0 && gas.Log.logConsole.console)
            {
                Commun.Logger($"yellow#Entrées Fixées{count}");
            }
        }
