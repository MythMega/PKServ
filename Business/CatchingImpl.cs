...
namespace PKServ.Business
{
    public class CatchingImpl(AppSettings appSettings, GlobalAppSettings globalAppSettings, DataConnexion dataConnexion)
    {
        ...
        private async Task CatchPokemon(BallThrowTreatement ballThrowTreatement)
        {
            ...
            msgResult += str2;
            appSettings.ballThrowHistory.Add(new BallThrowHistory { Ball = ballThrowTreatement.Ball, Pokemon = onePoke, User = new PKServ.User { Pseudo = ballThrowTreatement.User.Pseudo, Platform = ballThrowTreatement.User.Platform }, shiny = isShiny, price = ballThrowTreatement.Price, catchResult = isCaught, time = DateTime.Now, shownInOverlay_bar = false, isNew = false });
            if (isCaught)
                msgResult += $" [{ballThrowTreatement.User.Location.Name}]";
            if (!isCaught)
                return;
            appSettings.catchHistory.Add(new CatchHistory { Ball = ballThrowTreatement.Ball, Pokemon = onePoke, User = new PKServ.User { Pseudo = ballThrowTreatement.User.Pseudo, Platform = ballThrowTreatement.User.Platform }, shiny = isShiny, price = ballThrowTreatement.Price });
